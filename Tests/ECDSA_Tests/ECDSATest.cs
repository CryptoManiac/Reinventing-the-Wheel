...
public class ECDSATest
{
    ...
    public void ExpectedKeys(string tag, string expectedSecret, string expectedCompressed, string expectedUncompressed)
    {
        ...
        #region Serialization of keys
        {
            ...
            byte[] _public_key_compressed = new byte[curve.CompressedPublicKeySize];
            byte[] _public_key_uncompressed = new byte[curve.UncompressedPublicKeySize];
            ...
            if (!publicKey.Compress(_public_key_compressed))
            {
                throw new SystemException("Compression of the public key has failed");
            }

            if (!publicKey.Serialize(_public_key_uncompressed))
            {
                throw new SystemException("Serialization of the public key has failed");
            }

            secret = Convert.ToHexString(_secret_key);
            compressed = Convert.ToHexString(_public_key_compressed);
            uncompressed = Convert.ToHexString(_public_key_uncompressed);
        ...
        Console.WriteLine("{0} compressed public key: {1}", tag, compressed);
        Console.WriteLine("{0} uncompressed public key: 04{1}", tag, uncompressed);
        ...
        if (expectedCompressed != compressed)
        {
           // throw new SystemException("Compressed public key mismatch");
        }

        if (expectedUncompressed != uncompressed)
        {
    ...
    public void ExpectedSignature<HMAC_IMPL>(string tag, string message, string expected) where HMAC_IMPL: unmanaged, IMac
    {
    ...
    public void VerifySignature(string toCheck, string message)
    {
        byte[] signingHash = GetSigningHash(message);
