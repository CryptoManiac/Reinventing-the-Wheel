...
namespace Wheel.Crypto.Primitives.ByteVectors
{
    ...
    public struct ByteVec32
    {
        ...
        public unsafe void Load(Span<byte> from)
        {
            if (32 != from.Length)
            {
                throw new ArgumentOutOfRangeException(nameof(from), from.Length, "Span must be exactly 32 bytes long");
            }

            fixed (byte* target = &b00)
            {
                var to = new Span<byte>(target, 32);
                from.CopyTo(to);
            }
        }
        /// <summary>
        ...
        public unsafe readonly void Store(Span<byte> to)
        {
            if (32 != to.Length)
            {
                throw new ArgumentOutOfRangeException(nameof(to), to.Length, "Span must be exactly 32 bytes long");
            }

            fixed (byte* source = &b00)
            {
                var from = new Span<byte>(source, 32);
                from.CopyTo(to);
            }
        }
