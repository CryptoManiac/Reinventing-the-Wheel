...
namespace Wheel.Crypto.Primitives.ByteVectors
{
    ...
    public struct ByteVec128
    {
        ...
        public unsafe void Load(Span<byte> from)
        {
            if (128 != from.Length)
            {
                throw new ArgumentOutOfRangeException(nameof(from), from.Length, "Span must be exactly 128 bytes long");
            }

            fixed (byte* target = &b00)
            {
                var to = new Span<byte>(target, 128);
                from.CopyTo(to);
            }
        }
        ...
        public unsafe readonly void Store(Span<byte> to)
        {
            if (128 != to.Length)
            {
                throw new ArgumentOutOfRangeException(nameof(to), to.Length, "Span must be exactly 128 bytes long");
            }

            fixed (byte* source = &b00)
            {
                var from = new Span<byte>(source, 128);
                from.CopyTo(to);
            }
        }
